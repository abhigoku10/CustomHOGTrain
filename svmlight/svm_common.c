...
SVECTOR *create_svector(MY_WORD *words,char *userdefined,double factor)
{
  SVECTOR *vec;
  long    fnum,i;
  ...
  vec->words = (MY_WORD *)my_malloc(sizeof(MY_WORD)*(fnum));
  for(i=0;i<fnum;i++) { 
      vec->words[i]=words[i];
  }
	...
	SVECTOR is used */
{
    ...
    sum=(MY_WORD *)my_malloc(sizeof(MY_WORD)*veclength);
    sumi=sum;
    ai=a->words;
    bj=b->words;
    while (ai->wnum && bj->wnum) {
...
MODEL *read_model(char *modelfile)
{
  ...
  words = (MY_WORD *)my_malloc(sizeof(MY_WORD)*(max_words+10));
		    ...
		    long int *totwords, long int *totdoc)
{
  ...
  words = (MY_WORD *)my_malloc(sizeof(MY_WORD)*(max_words_doc+10));
  if(verbosity>=1) {
...
int parse_document(char *line, MY_WORD *words, double *label,
